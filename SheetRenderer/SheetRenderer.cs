...
public class RibbonController : ExcelRibbon
{
    ...
    private static List<SharedSheetDiffEntry> BuildSharedSheetDiffEntries(
    ...
    private static string BuildSharedSheetDiffText(
        ...
        SharedSheetDocument remoteDocument,
        SharedSheetDocument displayDocument = null,
        IReadOnlyList<SharedSheetDiffEntry> diffEntries = null)
    {
        ...
        IReadOnlyList<SharedSheetDiffEntry> entries = diffEntries ??
            BuildSharedSheetDiffEntries(baseDocument, localDocument, remoteDocument, displayDocument);
        ...
        lines.Add("changedCells: " + entries.Count);
    ...
    private static void SetSharedSheetSelectionDiff(
        ...
        SharedSheetDocument displayDocument = null)
    {
        ...
        List<SharedSheetDiffEntry> entries = BuildSharedSheetDiffEntries(
            baseDocument,
            localDocument,
            remoteDocument,
            displayDocument);
        item.DiffEntries = entries;
        item.DiffText = BuildSharedSheetDiffText(
            baseDocument,
            localDocument,
            remoteDocument,
            displayDocument,
    ...
    private static void ApplySharedSheetConflictResolutions(IEnumerable<SharedSheetConflictResolution> conflicts)
    {
        foreach (SharedSheetConflictResolution conflict in conflicts ?? Enumerable.Empty<SharedSheetConflictResolution>())
        {
            if (conflict == null)
            {
                continue;
            ...
            conflict.ApplyResolvedValue();
            FileLogger.Info(
                "[SharedReceiveConflictResolved] " +
                "sheetId=" + (conflict.SheetId ?? "") +
                " sheetName=" + (conflict.SheetName ?? "") +
                " rowId=" + (conflict.RowId ?? "") +
                " cell=" + (conflict.CellAddress ?? "") +
                " resolution=" + (conflict.ResolutionSource ?? "") +
                " base=" + FormatSharedCellValueForLog(conflict.BaseValue) +
                " local=" + FormatSharedCellValueForLog(conflict.LocalValue) +
                " remote=" + FormatSharedCellValueForLog(conflict.RemoteValue) +
                " resolved=" + FormatSharedCellValueForLog(conflict.ResolvedValue));
        }
    }
    ...
    private async Task<SharedManifestProbeResult> ProbeSharedProjectManifestAsync(
        GitLabShareInfo shareInfo,
        string projectId,
        string token)
    {
        ...
        string manifestPath = BuildSharedProjectManifestPath(projectId);
        TryLogSharedDiagnostic(
            "[SharedManifestProbe] start path=" + manifestPath +
        ...
        TryLogSharedDiagnostic(
            "[SharedManifestSnapshot] path=" + manifestPath +
            " projectId=" + shareInfo.ProjectId +
            " ref=" + refName +
            " final=" + result.State);

        if (result.State == SharedManifestProbeState.Indeterminate)
        {
            throw new InvalidOperationException(
        ...
        bool forceApplySharedValues = false)
    {
        ...
        foreach (SharedProjectManifestEntry entry in manifest.Sheets)
        {
            ...
            if (mergeResult.ConflictCount != mergeResult.Conflicts.Count)
            {
                throw new InvalidOperationException(
                    "Shared receive conflict resolution is unavailable. " +
                    "Verify that shared rowIds are present and compatible. " +
                    "sheetId='" + entry.SheetId + "', " +
                    "sheetName='" + sheet.Name + "'.");
            }

            pendingReceives.Add(new PendingSharedSheetReceive
            {
                SheetId = entry.SheetId,
                Sheet = sheet,
        ...
        List<SharedSheetConflictResolution> conflicts = pendingReceives
            .Where(x => x != null && x.MergeResult != null)
            .SelectMany(x => x.MergeResult.Conflicts ?? new List<SharedSheetConflictResolution>())
            .ToList();
        ...
        if (conflicts.Count > 0)
        {
            ...
            ApplySharedSheetConflictResolutions(conflicts);
            foreach (PendingSharedSheetReceive pendingReceive in pendingReceives)
            {
                pendingReceive.MergeResult.MergedDocument.Hash =
                    ComputeSharedSheetHash(pendingReceive.MergeResult.MergedDocument);
            }
        ...
        {
            progressReporter?.Invoke("解決済みの共有値競合を反映しました: " + conflictAppliedCount + " シート");
        }
    ...
    private sealed class SharedSheetMergeResult
    {
        ...
        public List<SharedSheetConflictResolution> Conflicts { get; } = new List<SharedSheetConflictResolution>();
    }

    private sealed class PendingSharedSheetReceive
    {
        ...
        SharedSheetDocument remoteDocument)
    {
        ...
        int conflictCount = 0;
        var conflicts = new List<SharedSheetConflictResolution>();
            ...
            for (int col = 0; col < columnCount; col++)
            {
                ...
                else
                {
                    ...
                    string cellAddress = displayRows.TryGetValue(rowId, out displayRow) && startColumn.HasValue
                        ...
                        : "?";
                    int conflictColumn = col;
                    conflicts.Add(new SharedSheetConflictResolution
                    {
                        SheetId = localDocument.SheetId,
                        SheetName = localDocument.SheetName,
                        RowId = rowId,
                        ...
                        ApplyValue = value => mergedRow[conflictColumn] = NormalizeSharedCellValue(value)
                    });
                }
            }
        ...
        var result = new SharedSheetMergeResult
        {
        ...
        Action<string> progressReporter = null)
    {
        ...
        foreach (SharedSheetSelectionItem item in itemList)
        {
            ...
            if (remoteDocument == null)
            {
                ...
                SetSharedSheetSelectionDiff(item, baseDocument, item.Document, null, displayDocument);
            ...
            if (mergeResult == null || mergeResult.ConflictCount > 0 || mergeResult.MergedDocument == null)
            {
                ...
                SetSharedSheetSelectionDiff(item, baseDocument, localDocument, remoteDocument, displayDocument);
                conflictSheetNames.Add(item.SheetName);
                continue;
            }
    ...
    private async Task UploadSharedSheetsAsync(
        ...
        GitLabShareInfo shareInfo,
        string token,
        SharedManifestProbeResult remoteManifestProbe,
        ...
        Action<string> progressReporter = null)
    {
        ...
        string refName = GetNormalizedShareRefName(shareInfo);
        SharedManifestCommitPlan manifestCommitPlan =
            SharedManifestSafety.CreateCommitPlan(remoteManifestProbe);
        SharedProjectManifest remoteManifest = remoteManifestProbe.Manifest;
        ...
        {
            { "action", manifestCommitPlan.Action },
            { "file_path", BuildSharedProjectManifestPath(projectId) },
            { "content", CreateSharedProjectManifestJsonText(updatedManifest) },
            { "encoding", "text" }
        };
        if (string.Equals(manifestCommitPlan.Action, "update", StringComparison.Ordinal))
        {
            manifestAction["last_commit_id"] = manifestCommitPlan.LastCommitId;
        }
        actions.Add(manifestAction);

        TryLogSharedDiagnostic(
            "[SharedManifestSnapshot] path=" + BuildSharedProjectManifestPath(projectId) +
            " projectId=" + shareInfo.ProjectId +
            " ref=" + refName +
            " commitAction=" + manifestCommitPlan.Action +
            " hasLastCommitId=" + (!string.IsNullOrWhiteSpace(manifestCommitPlan.LastCommitId)));
        ...
        catch (GitLabApiException ex)
        {
            SharedManifestCommitConflictKind conflictKind =
                SharedManifestSafety.ClassifyCommitConflict(ex, manifestCommitPlan.Action);
            TryLogSharedDiagnostic(
                "[SharedManifestCommitConflict] classified=" +
                (conflictKind != SharedManifestCommitConflictKind.None) +
                " kind=" + conflictKind +
                " action=" + manifestCommitPlan.Action +
                " status=" + ex.StatusCode +
                " url=" + ex.Url +
                " body=" + SharedManifestSafety.NormalizeForLog(ex.ResponseBody, 1000));
            if (conflictKind != SharedManifestCommitConflictKind.None)
            {
                throw new SharedManifestCommitConflictException(conflictKind, ex);
            }
        ...
        try
        {
            ...
            if (isPullUpdate)
            {
                ...
                if (!hasPullUpdate)
                {
                    ...
                    progressForm.ShowContinueButton(
                        "閉じる",
                        sharedReceiveResult.WasCanceled
                            ? "共有値の反映をキャンセルしました"
                            : "共有値確認が完了しました");
    ...
    private async Task ReceiveSharedSheetsAfterNewWorkbookCreatedAsync(GitLabShareInfo shareInfo, PullProgressForm progressForm)
    {
        ...
        ShowSharedReceiveConflictDialogIfNeeded(sharedReceiveResult);
        if (sharedReceiveResult.WasCanceled)
        {
            progressForm.SetStatusText("共有値の反映をキャンセルしました");
        }
    ...
    private async void OnShareButtonPressed(IRibbonControl control, string targetSheetName)
    {
        ...
        try
        {
            ...
            SharedManifestProbeResult remoteManifestProbe =
                await ProbeSharedProjectManifestAsync(
                    shareInfo,
        ...
        catch (SharedManifestCommitConflictException ex)
        {
            if (progressForm != null)
            {
                progressForm.CloseForm();
            ...
            ShowUserFacingError(dialogTitle, ex);
        }
        catch (Exception ex)
        {
            if (progressForm != null)
